...
public class IFMUwbRtlsIClientDual implements Runnable 
{

	private boolean debug = false;
	private int crunchifyRunEveryNSeconds = 150;
	...
	private static int setDataCount = 150;
	...
	public IFMUwbRtlsIClientDual()
	{
		
	}
	
	public IFMUwbRtlsIClientDual(int myInterval) 
	{
	...
	public IFMUwbRtlsIClientDual(String myFile, int myInterval) 
	{
		this.timestamp = new Timestamp(System.currentTimeMillis());
		this.curYYYY = this.sdfYYYY.format(this.timestamp);
		this.curMM = this.sdfMM.format(this.timestamp);
								...
								else if( dataCounter != 0 && ( dataCounter % (setDataCount+1) ) == 0 )
								{
									...
									if( this.isIFMUse == 1 )
									{
										if( ( (this.sbHeader.toString()).length() - (this.sbHeader.toString().replace(",", "")).length() ) == 665 )
										{
											...
											this.printLine(this.isViewYn, "[Send OK(1) "+ this.Poscoict_Svr_Ip +":"+ this.Poscoict_Svr_Port +"] >>>> " + this.sbHeader.toString() );
											this.setIFMLogFileWirte(this.isLogYn, 3, this.sbHeader.toString());
											
											
											this.out_2 = this.socket_2.getOutputStream();
											byte[] sendByte_2 = this.concat( this.convertIntToBytes( (this.sbHeader.toString().length() + 4) ), (this.sbHeader.toString()).getBytes() );
											this.out_2.write( sendByte_2, 0, sendByte_2.length );
											this.out_2.flush();
											this.printLine(this.isViewYn, "[Send OK(2) "+ this.Poscoict_Svr_Ip_2 +":"+ this.Poscoict_Svr_Port_2 +"] >>>> " + this.sbHeader.toString() );
										}
									}
    ...
    public static void main(String args[])
	{
		...
		IFMUwbRtlsIClientDual ifm = new IFMUwbRtlsIClientDual(150);
