...
public class IFUwbDataCombiClient implements Runnable 
{
	...
	private String ext_jdbc_driver = "";
	private String ext_jdbc_url = "";
	private String ext_db_id = "";
	private String ext_db_pw = "";
	...
	public IFUwbDataCombiClient()
	{
		
	}
	
	public IFUwbDataCombiClient(int myInterval) 
	{
		...
		try
		{
			...
			this.ext_jdbc_driver 	= properties.getProperty("datasource.poscoict.uwbextdatadb.db.driver");
			this.ext_jdbc_url 		= properties.getProperty("datasource.poscoict.uwbextdatadb.db.url");
			this.ext_db_id 			= properties.getProperty("datasource.poscoict.uwbextdatadb.db.id");
			this.ext_db_pw 			= properties.getProperty("datasource.poscoict.uwbextdatadb.db.pw");
	...
	public IFUwbDataCombiClient(String myFile, int myInterval) 
	{
		this.timestamp = new Timestamp(System.currentTimeMillis());
		this.curYYYY = this.sdfYYYY.format(this.timestamp);
		this.curMM = this.sdfMM.format(this.timestamp);
	...
	{	
    	while(shouldIRun)
    	{
        	...
        	StringBuffer sbExtQuery = new StringBuffer(7);
        	sbExtQuery.append(" INSERT INTO uwb_extcombination ");
        	sbExtQuery.append(" ( ");
        	sbExtQuery.append(" cranecode,tagid,posx,distance,hoist_height,subhoist_height ");
        	sbExtQuery.append(" ,ladle_status,ladle_num,cweight,mtlno,rweight,rmove,regdate ");
        	sbExtQuery.append(" )  ");
        	sbExtQuery.append(" VALUES  ");
        	sbExtQuery.append(" ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, now() ) ");
        	
        	String [] aryUwbData = null;
			...
			try 
			{
	    		...
	    		conExt = DriverManager.getConnection(this.ext_jdbc_url, this.ext_db_id, this.ext_db_pw);
				
				while (shouldIRun) 
				{	
					Thread.sleep(crunchifyRunEveryNSeconds);
				
					psExt = conExt.prepareStatement(" delete from uwb_extcombination where regdate <= (curdate() - INTERVAL 1 DAY) ");
					psExt.executeUpdate();
							...
							if( sDataLine != null && !"".equals(sDataLine) )
							{
								if( dataCounter != 0 && ( dataCounter % (setDataCount+1) ) > 0 )
								{
									...
									if( this.arrLen == 17 )
									{
											...
											this.tmp_rweight = this.isNullCheck(this.tmp_rweight, "");
											this.tmp_rmove = this.isNullCheck(this.tmp_rmove, "");
									...
									if( hmIFMData.size() < this.dsIFMDataCount )
									{
										...
										while( itrIFMDataKeys.hasNext() )
										{
											  ...
											  )
											{
												...
												if( strIFMDataTmp != null && !"".equals(strIFMDataTmp) )
												{
													aryUwbData = strIFMDataTmp.split(",");
													int extDataSize = aryUwbData.length;
													
													if( extDataSize >= 15 )
													{
														psExt = conExt.prepareStatement( sbExtQuery.toString() );
														psExt.setString(1, aryUwbData[3]);
														psExt.setString(2, aryUwbData[1]);
														psExt.setString(3, aryUwbData[4]);
														psExt.setString(4, aryUwbData[5]);
														psExt.setString(5, aryUwbData[6]);
														psExt.setString(6, aryUwbData[7]);
														psExt.setString(7, aryUwbData[8]);
														psExt.setString(8, aryUwbData[9]);
														psExt.setString(9, aryUwbData[12]);
														psExt.setString(10, aryUwbData[13]);
														
														if( "-99".equals(aryUwbData[14]) )
														{
															psExt.setString(11, "");
														}
														else
														{
															psExt.setString(11, aryUwbData[14]);
														}
														
														if( extDataSize == 16 )
														{
															if( "-99".equals(aryUwbData[15]) )
															{
																psExt.setString(12, "");
															}
															else
															{
																psExt.setString(12, aryUwbData[15]);
															}
														}
														else
														{
			...
			finally
			{
				try
				{
					...
					if( psExt != null ) psExt.close();
	        		if( conExt != null ) conExt.close();
    ...
    public static void main(String args[])
	{
		...
		IFUwbDataCombiClient ifm = new IFUwbDataCombiClient(50);
